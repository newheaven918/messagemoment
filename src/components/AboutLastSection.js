...
const AboutLastSection = ({ bg, classes = '', title, des, pr, imgClass = '', main = '', downarrow = false }) => {
  return (
    <div className={`border-t bg-gradient-to-b from-blue-10 from-0% via-blue-0 via-60% to-white to-90% ${main}`}>
      ...
      <div className={`flex sm:justify-center ${classes} lg:items-center gap-6 lg:gap-[95px] max-lg:flex-col`}>
        <div className="flex sm:justify-center sm:items-center">
          <Image src={bg} className={imgClass} alt="Image" />
        </div>
        <div className="flex sm:justify-center sm:items-center flex-col  ">
          <h3 className={`lg:text-[60px] text-[34px] font-bold lg:leading-[70px] pl-[2px] text-dark max-w-[576px] tracking-[1px] ${pr} lg:tracking-[2.38px] max-md:tracking-[-2px] leading-[1.17]`}>{title}</h3>
          <p className="font-jetBrain text-[13px] lg:text-[17px] ml-[-2px] pt-[2px] mt-[19px] max-w-[570px] pr-[31px] leading-[22px]">{des}</p>
        </div>
      </div>
    </div>
  );
