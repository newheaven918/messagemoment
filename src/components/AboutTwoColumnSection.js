...
const AboutTwoColumnSection = ({ bg, classes = '', title, des, pr, pl, imgClass = '', main = '', downarrow = false }) => {
  return (
    <div className={`border-t bg-gradient-to-b from-blue-10 from-0% via-blue-0 via-60% to-white to-90% ${main}`}>
      {downarrow && (
      ...
      <div className={`flex sm:justify-center ${classes} lg:items-center gap-6 lg:gap-[19px] max-lg:flex-col`}>
        <div className="flex sm:justify-center sm:items-center ">
          <Image src={bg} className={imgClass} alt="Image" />
        </div>
        <div className="flex sm:justify-center sm:items-center flex-col">
          <h3 className={`lg:text-[60px] text-[34px] mb-[2px] font-bold lg:leading-[70px] text-dark max-w-[576px] tracking-[2px] ${pr} ${pl} lg:tracking-[2.22px] max-md:tracking-[-2px] leading-[1.17]`}>{title}</h3>
          <p className={`font-jetBrain text-[13px] lg:text-[17px] font-medium mt-[18px] max-w-[576px] pr-[20px] leading-[22px] ${pl} `}>{des}</p>
        </div>
      </div>
    </div>
  );
