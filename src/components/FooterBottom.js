...
const FooterBottom = ({ classes = '', textColor = 'text-darkGrey' }) => {
  ...
  return (
    <div className={`flex flex-col lg:flex-row items-center justify-between w-full max-w-[1440px] mx-auto px-4 pt-[3px] ${classes}`}>
      <p className={`${textColor} font-medium text-[10px] lg:text-[13px] order-1 lg:order-none mt-[31px] lg:mt-0 mb-[7px] ml-[43px] lg:mb-0`}>Copyright © {currentYear} MessageMoment. All rights reserved.</p>
      <SocialIcons textColor={textColor} pR={`bt-[1px]`} />
      <p className={`${textColor} font-medium text-[10px] lg:text-[13px] lg:w-[337.9px] text-right flex justify-center order-1 lg:order-none lg:justify-end items-center gap-[5px] lg:gap-[10px] mr-[44px]`}>
