  ...
  return (
    <div className="relative">
      <div ref={carousel} className="steps-desktop sliders flex gap-[10px] sm:gap-5 overflow-x-scroll scroll-smooth px-[90px] sm:px-0 sm:overflow-x-hidden -mx-[15px] sm:mx-auto pb-[23px]">
        {steps.map((step, i) => {
          return (
            <div className={`step bg-white rounded-[20px] min-w-[180px]  sm:min-w-0 relative pt-[16px] pl-[31px] sm:pl-[23px] sm:pr-[24px] sm:pt-[24px] sm:pb-[26px] cursor-pointer ${index === i ? 'sm:w-[220px] lg:w-[260px] xl:flex-1 sm:ml-[4px]' : 'sm:w-[216px] lg:w-[260px] bg-opacity-20 text-center'}`} key={step.id} onMouseEnter={() => handleHoverFunction(i)}>
              <h2 className={`font-bold text-[34px] lg:text-[60px] absolute left-[30px] pl-[1px] pb-[2px] top-[6px] ${index === i ? 'text-blue' : 'text-white opacity-20'}`}>{step.id}</h2>
              <Image src={step.img} alt="step" className={`w-[60px] h-[60px] lg:w-[80px] lg:h-[80px] mt-[37px] object-cover ${index === i ? 'mx-auto sm:ml-[45px] lg:ml-[67px]' : 'mx-auto'} mb-[16px] `} />
              <span className={`font-jetBrain text-[13px]  ${index === i ? '  sm:pl-[7px] text-darkGrey' : 'text-white ' }`}>{step.step}</span>
              <p className={`font-bold text-[17px]  lg:leading-[26px] lg:text-[26px] pt-[1px] pb-2 ${index === i ? 'lg:leading-[26px] pl-[7px] text-dark' : 'pl-[wpx] text-white'}`}>{step.stepName}</p>
